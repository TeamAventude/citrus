...
import { Search, Calendar, Filter, AlertTriangle } from 'lucide-react';
...
export const SimpleToolHistoryDashboard: React.FC = () => {
  ...
  const usabilityDecision = useMemo((): UsabilityDecision | null => {
    ...
    // Check repair frequency threshold
    if (repairAnalytics.totalRepairs > 3) {
      reasons.push(`⚠️ Excessive repairs (${repairAnalytics.totalRepairs} repairs) - indicates reliability issues`);
      status = 'Not Usable';
    }

    // Check repair cost ratio with detailed breakdown
    if (repairAnalytics.repairToValueRatio > 0.7) {
      reasons.push(`💰 Repair costs (${Math.round(repairAnalytics.repairToValueRatio * 100)}%) exceed 70% of tool value - economically unfeasible`);
      status = 'Not Usable';
    } else if (repairAnalytics.repairToValueRatio > 0.5) {
      reasons.push(`⚡ Repair costs (${Math.round(repairAnalytics.repairToValueRatio * 100)}%) approaching 50% of tool value - monitor closely`);
    }

    // Check latest QC status with specific guidance
    ...
    if (latestQC && latestQC.details.status === 'Fail') {
      if (latestQC.type === 'eol') {
        reasons.push(`🔚 End-of-life assessment failed - tool deemed unsafe or unreliable`);
      } else {
        reasons.push(`❌ Latest Quality Control assessment failed - requires immediate attention`);
      }
      status = 'Not Usable';
    }

    // Check latest repair status with specific recommendations
    if (repairAnalytics.lastRepairStatus === 'Fail') {
      reasons.push(`🔧 Latest repair attempt failed - may require specialized service or replacement`);
      status = 'Not Usable';
    }

    // Check for safety-related concerns
    const safetyKeywords = ['safety', 'overheating', 'leak', 'pressure', 'motor failure', 'critical failure'];
    const recentRepairs = toolHistory.events
      .filter(e => e.type === 'repair' || e.type === 'return')
      .slice(-3); // Last 3 repair/return events
    
    const hasSafetyConcerns = recentRepairs.some(event => 
      safetyKeywords.some(keyword => 
        (event.details.repairNote?.toLowerCase().includes(keyword) || 
         event.details.returnCondition?.toLowerCase().includes(keyword) ||
         event.details.eolReason?.toLowerCase().includes(keyword))
      )
    );

    if (hasSafetyConcerns) {
      reasons.push(`⚠️ Safety concerns identified in recent events - prioritize user safety`);
      status = 'Not Usable';
    }

    // Check borrowing patterns for reliability insights
    if (borrowingAnalytics) {
      if (borrowingAnalytics.overdueCount > 2) {
        reasons.push(`📅 Multiple overdue returns (${borrowingAnalytics.overdueCount}) - may indicate tool reliability issues`);
      }
      
      // Check for frequent short-term returns (potential reliability issues)
      const shortReturns = toolHistory.events
        .filter(e => e.type === 'return')
        .filter(e => e.details.returnCondition && 
          !e.details.returnCondition.toLowerCase().includes('good') &&
          !e.details.returnCondition.toLowerCase().includes('excellent')
        );
      
      if (shortReturns.length > 2) {
        reasons.push(`🔄 Multiple returns with issues reported - pattern suggests declining reliability`);
        if (status === 'Usable' && repairAnalytics.totalRepairs > 1) {
          status = 'Not Usable';
        }
      }
    }

    // Positive indicators for usable tools
    if (status === 'Usable') {
      if (repairAnalytics.totalRepairs === 0) {
        reasons.push(`✅ No repairs needed - excellent reliability record`);
      } else if (repairAnalytics.totalRepairs <= 2 && repairAnalytics.repairToValueRatio < 0.3) {
        reasons.push(`✅ Minimal repair history with successful outcomes - good maintenance record`);
      } else {
        reasons.push(`✅ All checks passed - tool meets usability standards`);
      }

      // Add maintenance recommendations
      if (repairAnalytics.totalRepairs > 0) {
        const daysSinceLastRepair = repairAnalytics.lastRepairDate ? 
          Math.floor((new Date().getTime() - new Date(repairAnalytics.lastRepairDate).getTime()) / (1000 * 60 * 60 * 24)) : 
          null;
        
        if (daysSinceLastRepair && daysSinceLastRepair > 365) {
          reasons.push(`🔍 Consider preventive maintenance - last repair was ${Math.floor(daysSinceLastRepair / 365)} year(s) ago`);
        }
      }
    }
    ...
    if (toolHistory.events.length < 5) {
      confidence = 'Medium';
      reasons.push(`📊 Limited event history - recommendation based on available data`);
    } else if (toolHistory.events.length > 15) {
      confidence = 'High';
    }

    if (repairAnalytics.totalRepairs > 0 && !repairAnalytics.lastRepairDate) {
      confidence = 'Low';
    }
  ...
  }, [toolHistory, repairAnalytics, borrowingAnalytics]);
